<template>
  <div align="center">
    <div v-if="chartDataS.labels.length" class="chart-container">
      ...
      <div class="averages">
        <div>
          <strong>자유적립식 저축 금리 평균:</strong>
          {{ averagesS.averageInterestRate }}%
        </div>
        <div>
          <strong>자유적립식 최고 우대 금리 평균:</strong>
          {{ averagesS.averageMaxRate }}%
        </div>
      </div>
    </div>
    ...
    <div v-if="chartDataF.labels.length" class="chart-container">
      ...
      <div class="averages">
        <div>
          <strong>정액적립식 저축 금리 평균:</strong>
          {{ averagesF.averageInterestRate }}%
        </div>
        <div>
          <strong>정액적립식 최고 우대 금리 평균:</strong>
          {{ averagesF.averageMaxRate }}%
        </div>
      </div>
    </div>
  </div>
...
// 차트 데이터 및 평균 계산
const chartDataS = computed(() => createChartData(props.data, "S"));
const chartDataF = computed(() => createChartData(props.data, "F"));

const averagesS = computed(() => calculateAverage(props.data, "S"));
const averagesF = computed(() => calculateAverage(props.data, "F"));
